...
library(tidyverse)
#a
annotation <- read_tsv("../data/FileS9.txt",col_names = c("GeneID","description")) #path will be different for students
head(annotation)
DEgene.trt <- read_csv("DEgenes.trt.csv")
head(DEgene.trt)
colnames(DEgene.trt)[1] <- "GeneID"
DE.trt.annotated <- left_join(DEgene.trt, annotation, by="GeneID") %>% arrange(FDR)
head(DE.trt.annotated,10)

#b
DEgene.interaction <- read_csv("DEgenes.interaction.csv")
colnames(DEgene.interaction)[1] <- "GeneID"
head(DEgene.interaction)
DE.interaction.annotated <- left_join(DEgene.interaction,annotation,by="GeneID")
...
library(goseq)
go.terms <- read_tsv("../data/FileS11.txt",col_names=FALSE)
head(go.terms)
colnames(go.terms) <- c("GeneID","GO")
head(go.terms)

expressed.genes <- read_tsv("../data/internode_expressed_genes.txt")
head(expressed.genes)
names(expressed.genes) <- "GeneID"

gene.lengths <- read_tsv("../data/Brapa_CDS_lengths.txt")
head(gene.lengths)
