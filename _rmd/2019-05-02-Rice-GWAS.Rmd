...
data.pheno.mds %>%
  ggplot(aes(x=Region,y=`Amylose content`)) +
  geom_boxplot()
```
...
pl <- pl + facet_wrap(facets= ~ assignedPop, ncol=3) # a separate plot ("facet") for each region, arranged in 3 columns
pl <- pl + ggtitle("Aluminum Tolerance") #add a title
pl #display the plot
qplot(x=assignedPop,y=Alu.Tol,data=data.pheno.mds,geom="boxplot")
qplot(x=assignedPop,y=Alu.Tol,data=data.pheno.mds,geom="violin")
```
...
```{r Ex7a,echo=FALSE,eval=FALSE}
data.pheno.mds %>% group_by(assignedPop) %>%
  summarize(mean.Aluminum_tolerance=mean(`Alu.Tol`,na.rm=T)) %>%
  arrange(desc(mean.Aluminum_tolerance))
```

```{r Ex7b, results='hide',eval=FALSE}
...
snpinfo <- data.frame(snp=colnames(data.geno)[-1],
                      chr=sub("([0-9]+)_.+","\\1",colnames(data.geno)[-1]),
                      pos=sub("[0-9]+_([0-9]+)$","\\1",colnames(data.geno)[-1]))
write.csv(snpinfo,"../data/RiceSNPData/snpInfo.csv")
```

Finally we are ready to perform a GWAS.  Please run the commands below to set up the data for analysis.

```{r, eval=FALSE, echo=FALSE}
snpinfo <- read.csv("../data/RiceSNPData/snpInfo.csv",row.names=1)
```
...
grep("3_30449857",colnames(snps3))
