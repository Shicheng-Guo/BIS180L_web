...
The _TidyVerse_ contains convenient functions for data manipulation.
...
How many rows were in the original data set and how many are in your filtered data set?
...
There were two researchers that measured the data.  Perhaps we only trust the data gathered by one researcher.  Find the names of the researchers and then filter to only keep the data measured by the second researcher in the list.
...
Want more info? see [filter](http://r4ds.had.co.nz/transform.html#filter-rows-with-filter) in R 4 data science.
...
Practice by using select to create a new tibble that only has the hypcotyl and internade date along with species, accession, and trt.
...
Want more info? See [select](http://r4ds.had.co.nz/transform.html#select-columns-with-select) in R 4 data science.
...
Want more info? See [pipes](http://r4ds.had.co.nz/pipes.html) in R 4 data science.
...
Dan_leaf_data <- tomato %>% #start with the tomato data; pipe it to the next command.  final results will go into Dan_leaf_data
  filter(who=="Dan") %>%
...
Want more info? see [arrange](http://r4ds.had.co.nz/transform.html#arrange-rows-with-arrange) in R 4 data science.
...
Want more info? see [mutate](http://r4ds.had.co.nz/transform.html#add-new-variables-with-mutate) in R 4 data science.
