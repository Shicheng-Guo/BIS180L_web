...
GxE_cor <- cor(t(gene_exp300))
GxE_cor %>% round(3) %>% as_tibble() %>% select(1:10) %>% head()
diag(GxE_cor) <- 0
...
GxE_MR %>% as_tibble() %>% select(1:10) %>% head()

genes_adj_MR4 <- GxE_MR <= 4
diag(genes_adj_MR4) <- 0

genes_adj_MR10 <- GxE_MR <= 10
...
```{r plotdensity, echo=FALSE, results='hide'}
