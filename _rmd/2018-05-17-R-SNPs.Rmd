...
library(tidyverse)
vcf.data <- read_tsv("../data/IMB211_R500.vcf",na = c("","NA","."),comment="#",col_names = FALSE)
head(vcf.data)
```
...
vcf.data.good <- vcf.data %>% filter(QUAL>=40)
```
...
pl <- ggplot(vcf.data.good,aes(x=POS,y=tot.depth,color=category,pch=category))
pl <- pl + geom_point()
pl
pl + xlim(20000000,25000000)
```
