...
plant_with_worm_orthologs <- plant_with_recip %>%
  filter(query_id==subject_id.worm_plant,
         E.plant_worm < 1e-04,
         E_diff.plant_worm < -2)
...
worm_with_recip <- left_join(worm_plant_best, plant_worm_best,
  ...
  select(query_id, subject_id, subject_id.plant_worm, everything()) # rearrange columns 

head(worm_with_recip,10)
```


```{r, eval=FALSE, include=FALSE}
worm_with_plant_orthologs <- worm_with_recip %>%
  filter(query_id==subject_id.plant_worm,
         E.worm_plant < 1e-04,
         E_diff.worm_plant < -2)
nrow(worm_with_plant_orthologs) #1886
```
...
```{r, eval=FALSE, include=FALSE}
worm_plant_best %>% ggplot(aes(x=Score,y=len)) + geom_point()
```
...
```{r, eval=FALSE, include=FALSE}
worm_plant_best %>% ggplot(aes(x=Score,y=len,color=pct_ident)) + geom_point()
```
...
```{r, eval=FALSE, include=FALSE}
worm_plant_best %>% filter(E!=0) %>% 
  ggplot(aes(x=Score,y=E)) + 
  geom_point() +
  scale_y_log10()
