...
```{bash, eval=FALSE}
qiime tools import \
  --input-path gg_13_8_otus/rep_set_aligned/97_otus.fasta \
...
```{bash, eval=FALSE}
 # table of OTUs
qiime tools export \
  --input-path Data/RiceTable-dn-99.qza \
  --output-path qiime_export

# sequences
qiime tools export \
  --input-path Data/RiceRep-seqs-dn-99.qza \
  --output-path qiime_export
```
...
```{r}
rice <- import_biom(BIOMfilename = "~/Assignments/Assignment_9/qiime_export/feature-table.biom",
                    refseqfilename = "~/Assignments/Assignment_9/qiime_export/dna-sequences.fasta")
```
