...
```{r GFF2GTF_JM, eval=FALSE, echo=FALSE}
library(tidyverse)
library(rtracklayer)
gff <- import.gff("../data/Brapa_gene_v1.5.gff.gz")
gff #take a look

#create a column "gene_id" that contains the gene name for every entry
gff$gene_id <- ifelse(is.na(gff$ID),gff$Parent,gff$ID)

export(gff,"../data/Brapa_gene_v1.5.gtf",format="gtf")
```
...
```{r Rsubread_jm, eval=FALSE, echo=FALSE}
library(Rsubread)
readCounts <- featureCounts(
...
```{r convert_to_tibble_friendly, echo=FALSE, eval= FALSE, results='hide'}
counts.data <- read.delim("../data/gh_internode_counts.tsv")
counts.data <- counts.data %>% rownames_to_column(var="gene_id") %>% select(gene_id, everything())
write_tsv(counts.data,"../data/gh_internode_counts2.tsv")
```
...
counts.melt <- counts.data %>% gather(key="sample",value="counts",-gene_id)
head(counts.melt)
pl <- ggplot(data=counts.melt,aes(x=counts))
pl <- pl + facet_wrap( ~ sample)
...
pairs(log10(counts.data[1:1000,-1]))
pairs(log10(counts.data[1:1000,2:7]))
...
boxplot(log2(counts.data[,-1] + 1),main="counts before transformation",ylab = "log2(counts)")
...
DEgene.interaction <- topTags(interaction.lrt,n = Inf, p.value = 0.01)$table
