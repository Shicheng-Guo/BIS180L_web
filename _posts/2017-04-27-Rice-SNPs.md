...
* Start a new R project for Assignment 4 by cloning this repository using Rstudio.
...
What if we want to combine the `fruit.color` and `fruit.shape` objects into a single data frame?  We need to make sure that the correct rows are combined.  The `merge` function helps us do that:
```r
...
**EXERCISE 2:**
* Use the `read.csv()` `head()` and `summary()` functions that you learned earlier to import and look at this file.  Import the file into an object called "data.pheno".  __Be sure to include the `row.names=1` argument when you call `read.csv()`__
...
*Hint 1* use `color=` argument to ggplot or ggplot to color the point by the different traits

*Hint 2* use `size=I(3)` as an argument to increase the point size (you can play with different values)

*Hint 3* when plotting the Region data, the colors will be easier to interpret if you include the following at the end of the line with your ggplot command: ` + scale_color_brewer(type="div")` This specifies that a diverging set of colors is used.  (Try plotting with and without this).
...
The next step is to take the data from "wide" format to "long" format.  `ggplot()` needs one observation per row, whereas now we have 5 observations per row.  We use `melt()` as described at the beginning of this tutorial.
...
We will use some of the objects that you have created today in Tuesday's lab, so lets save them in an .Rdata file for easy loading on Tuesday
