...
_Note: one of the libraries in the tutorial has been renamed from `shinyapps` (on your computer) to `rsconnect` (in the online tutorial).  Either substitute `shinyapps` anytime you see `rsconnect` or install rsconnect `install.packages("rsconnect")`_
